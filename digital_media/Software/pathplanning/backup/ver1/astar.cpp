...
aStar::aStar(vector<vector<Pixel> > * aRoadMap, int aStartX, int aStartY, int aEndX, int aEndY)
{
   ...
   // printMap();
    heightFactor = 2./height; //That is if the lenght and height of the picture is 2
    widthFactor = 2./width;
...
void aStar::findShortestPath()
{
    ...
    tmpString = std::to_string(current->x*widthFactor) + "," + std::to_string(current->y*heightFactor);
    wayPoints.push_back(tmpString);
    while (current->backPointer != NULL) //Start will have a nullptr, so stop when it's reached
	{
       ...
       if(current->x - current->backPointer->x  != deltaX|| current->y - current->backPointer->y != deltaY)
       {
           ...
           waypoint = std::to_string(current->x*widthFactor) + ","+ std::to_string(current->y*heightFactor);
    ...
    tmpString = to_string(current->x*widthFactor) + "," + to_string(current->y*heightFactor);
    wayPoints.push_back(tmpString);
    ...
    cout << "Here comes the waypoints" << endl;
