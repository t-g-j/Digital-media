...
aStar::aStar(vector<vector<Pixel> > * aRoadMap, int aStartX, int aStartY, int aEndX, int aEndY, double trueWidth)
{
    walkAble = 0;
    roadmap = aRoadMap;
	endX = aEndX;
	...
	startX = aStartX;
	startY = aStartY;
    height = (*aRoadMap).size();
    width = (*aRoadMap)[0].size();
    cout << "width: " << width << endl;
    cout << " height: " << height << endl;
    (*roadmap)[startX][startY].value = startPos;
    //(*roadmap)[endX][endY].value = desPos;
   // printMap();
    pixelsToMeters = trueWidth / width;
    cout << "PixelsToMeters factor: " << pixelsToMeters << endl;
...
void aStar::findShortestPath()
{
	...
	{
        shortPX.push_back(current->x/**pixelsToMeters*/); //Store x and y values
        shortPY.push_back(current->y/**pixelsToMeters*/);
       if(current->x - current->backPointer->x  != deltaX|| current->y - current->backPointer->y != deltaY)
       {
           wayPointsX.push_back(current->x/**pixelsToMeters*/);
           wayPointsY.push_back(current->y/**pixelsToMeters*/);
            (*inputMap)[current->x][current->y].value = wayPointVal;
           deltaX = current->x - current->backPointer->x;
           deltaY = current->y - current->backPointer->y;
       }
	...
	{
        (*inputMap)[shortPY[i]][shortPX[i]].value = shortestPath;
	}
    (*inputMap)[startX][startY].value = startPos;
    (*inputMap)[endX][endY].value = wayPointVal;
...
void aStar::updateAdjecentSquares(Pixel * currentPixel)
{
        ...
        for(int j = -1; j<2; j++)
    {
			...
			if (withinMap(currentPixel->x + j, currentPixel->y + i))
			{
				...
				if (!(j == 0 && i == 0))
				{
                if ((*inputMap)[currentPixel->x+j][currentPixel->y+i].value == 0) //WAS (*roadMap) BEFORE trying to quickfix
				{
 ...
 void aStar::printMap()
 {
     ...
     for (int yi = 0; yi < height; yi++)
     {
         for (int xi = 0; xi < width; xi++)
         {
             cout << (*inputMap)[yi][xi].value << " ";
             out << (*inputMap)[yi][xi].value << " ";
		 }
		 cout << endl;
         out << endl;
	 }
